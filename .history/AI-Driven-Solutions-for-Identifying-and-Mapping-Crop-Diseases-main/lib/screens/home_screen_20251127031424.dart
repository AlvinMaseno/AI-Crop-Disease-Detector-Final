    ...
    return Scaffold(
      ...
      floatingActionButton: _selectedIndex == 0
          ? FloatingActionButton.extended(
              onPressed: () =>
                  context.push('/upload'), // Use push to keep back button
              label:
                  Text(context.watch<AppProvider>().translate('diagnoseCrop')),
...
class _HomeTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dateStr = DateFormat('EEEE, d MMMM')
        .format(DateTime.now()); // Requires intl package, or use string logic
            ...
            Padding(
              ...
              child: Consumer<AppProvider>(
                builder: (context, appProvider, child) {
                  ...
                  final healthy =
                      reports.where((r) => r.severity == 'Low').length;

                  return Row(
                    children: [
                      Expanded(
                        child: _buildStatCard(theme, 'Scans', '$total',
                            Icons.qr_code_scanner, Colors.blue),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: _buildStatCard(theme, 'Healthy', '$healthy',
                            Icons.check_circle_outline, Colors.green),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: _buildStatCard(theme, 'Critical', '$critical',
