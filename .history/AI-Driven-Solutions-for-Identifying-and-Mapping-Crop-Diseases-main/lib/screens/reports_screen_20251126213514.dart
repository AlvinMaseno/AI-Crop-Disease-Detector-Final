...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _handleDownloadReports() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'crop_disease_reports.pdf'
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _searchQuery.isNotEmpty || _selectedFilter != 'All' ? 'No reports found' : 'No reports yet',
              style: theme.textTheme.headlineSmall?.copyWith(color: Colors.grey.shade600, fontWeight: FontWeight.w600),
            ),
            const SizedBox(height: 8),
            Text(
              _searchQuery.isNotEmpty || _selectedFilter != 'All' ? 'Try adjusting your criteria' : 'Start by uploading an image',
              style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey.shade500),
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 24),
            if (_searchQuery.isEmpty && _selectedFilter == 'All')
              CustomButton(text: 'Upload First Image', onPressed: () => context.go('/upload'), icon: Icons.camera_alt)
            else
              CustomButton(text: 'Clear Filters', onPressed: () {
                  _searchController.clear();
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    Container(
                      width: 48, height: 48,
                      decoration: BoxDecoration(color: _getSeverityColor(report.severity).withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(color: _getSeverityColor(report.severity).withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                      child: Text(report.severity, style: TextStyle(color: _getSeverityColor(report.severity), fontWeight: FontWeight.w600, fontSize: 12)),
                    ),
                  ],
                ),
              ),
  ...
  // Generate a single PDF for sharing from the details view
      ...
      return pw.Center(
        child: pw.Column(
          children: [
            ...
            pw.SizedBox(height: 20),
            pw.Text("Crop: ${report.cropName}"),
            pw.Text("Severity: ${report.severity}"),
            pw.Text("Confidence: ${report.formattedConfidence}"),
            pw.SizedBox(height: 20),
