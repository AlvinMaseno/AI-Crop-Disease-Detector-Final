...
/// Formats raw ML class names (e.g., 'Cashew__anthracnose' to 'Cashew Anthracnose').
String formatPredictionName(String rawName) {
  // Replace double underscores with space
  String cleanedName = rawName.replaceAll('__', ' ');
  // Replace single underscores with space (e.g. 'leaf_miner' -> 'leaf miner')
  cleanedName = cleanedName.replaceAll('_', ' ');

  // Capitalize words
  return cleanedName.split(' ').map((word) {
    if (word.isEmpty) return '';
    // Handle special case for "Corn_(maize)" formatting if needed, or just basic capitalization
    return word[0].toUpperCase() + word.substring(1).toLowerCase();
  }).join(' ');
}

/// Extracts the Crop Name from the raw prediction string (e.g., 'Cashew__anthracnose' -> 'Cashew')
String extractCropName(String rawName) {
  // The backend uses format "Crop__Disease"
  if (rawName.contains('__')) {
    String crop = rawName.split('__')[0];
    // Clean up "Corn_(maize)" to "Corn (Maize)" or similar if preferred
    return crop.replaceAll('_', ' ');
  }
  return "Unknown Crop";
}

/// Provides treatment suggestions based on the specific diseases in your model.
...
class ImagePreviewWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: kIsWeb
          ? Image.network(selectedXFile.path,
              fit: BoxFit.cover,
...
class PredictionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Extract crop name for display if desired, or just use it in the logic
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Chip(
                        label: Text(cropName),
                        backgroundColor:
                            Colors.green[100]), // Show detected crop
                  ],
                ),
                ...
                Row(children: [
                  ...
                  Text("Confidence Score: $confidenceString%",
                      style: const TextStyle(
...
class _UploadScreenState extends State<UploadScreen> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.containsKey("prediction") && data.containsKey("confidence")) {
          ...
          // 1. Get Treatment using improved logic
          ...
          final newReport = DiseaseReport(
            ...
            severity: confidenceScore > 90 ? "High" : "Moderate",
            cropName: detectedCrop, // NOW USING ACTUAL DETECTED CROP
            treatmentRecommendation: recommendation,
          );

          // 4. Save to Provider
          Provider.of<AppProvider>(context, listen: false).addReport(newReport);
  ...
  void generateReport() {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Generating PDF report... (Feature coming soon!)')));
  }
        ...
        child: Column(
          children: [
            if (!hasImage && !hasResult)
              Column(
                children: [
                  const Text("Upload a plant leaf image for diagnosis.",
                      style:
              ...
              Column(
                children: [
                  Card(
                      elevation: 5,
                      child:
                          ImagePreviewWidget(selectedXFile: _selectedXFile!)),
