  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Card(elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // --- FIX 1: HEADER OVERFLOW ---
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween, 
          children: [
            Expanded( // Forces Text to shrink if needed
              child: Text("🌿 Diagnosis Complete", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Theme.of(context).primaryColor)),
            ),
            Padding( // Added padding for better visual spacing next to text
              padding: const EdgeInsets.only(left: 8.0),
              child: Chip(label: Text(cropName), backgroundColor: Colors.green[100]),
            ),
          ],
        ),
        // -----------------------------
      ...
      // --- FIX 2: BUTTON OVERFLOW ---
      Row(
        children: [
          Expanded(
            child: OutlinedButton.icon(onPressed: onGenerateReport, icon: const Icon(Icons.file_download), label: const Text("Report")),
          ),
          const SizedBox(width: 16),
          Expanded(
            child: ElevatedButton.icon(onPressed: onAnalyzeAnother, icon: const Icon(Icons.refresh), label: const Text("Analyze Another")),
          ),
        ],
      ),
      // ------------------------------
  ...
  Future<void> analyzeImage() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (conf < 45.0) {
          _showErrorDialog("Uncertain Result (${conf.toStringAsFixed(1)}%).\nPlease retake the photo closer to the leaf.");
      ...
      } else { _showErrorDialog("Server Error: ${response.statusCode}"); }
    } catch (e) { _showErrorDialog("Network Error. Check your server connection.\nDetails: $e"); } 
    finally { setState(() => _isAnalyzing = false); }
  }

  void _showErrorDialog(String message) => showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text("Error"), content: Text(message), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("OK"))]));
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          if (hasImage && !hasResult && !_isAnalyzing) ...[
            ...
            const SizedBox(height: 20),
            ElevatedButton.icon(onPressed: analyzeImage, icon: const Icon(Icons.search), label: const Text("Analyze Image"), style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 50))),
